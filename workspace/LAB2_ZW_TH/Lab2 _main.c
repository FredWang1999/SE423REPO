// Ze Wang is commenting this Code. Comments marked by my initials ZW
//#############################################################################
...
int16_t read_button = 0;

//ZW set state of led, first 5 bits for 5 leds
...
//ZW read switches, first 4 bits for each switch
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, LAUNCHPAD_CPU_FREQUENCY, 250000);//21.47s maximum for 32 bit timer at 200Mhz
...
//    init_serialSCID(&SerialD,115200);
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
			...
			serial_printf(&SerialA,"Timer2 Calls %ld Num SerialRX: %ld read_botton %d \r\n",numTimer2calls,numRXA,read_button);
            UART_printfLine(1,"Timer2 Calls %ld",CpuTimer2.InterruptCount);
            UARTPrint = 0;//set to zero so the function is not called over and over again
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //ZW variable that counts up when timer is called
    numTimer2calls ++;
	...
	//ZW set led based on button state
	read_button = ReadSwitches();
	SetLEDsOnOff(read_button);
}
