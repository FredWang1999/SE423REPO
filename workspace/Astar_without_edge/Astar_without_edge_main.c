...

{
    ...
    if (LADARpingpong == 1) {
        ...
        for (LADARi = 0; LADARi < 228; LADARi++) {
            ...
            LiDAR_X += ladar_pts[LADARi].x;

            LiDAR_Y += ladar_pts[LADARi].y;

            LiDAR_Dist += ladar_data[LADARi].distance_ping;
...

            {
...

                {
                    ...
                    obsIndex = (int)((11-LiDAR_Y)*11 + LiDAR_X - 5);

                    if ((testMap[obsIndex] == '0') && (obstacleCandidate[obsIndex].counter < SOLID_OBS_NUM))

                    {

                        obstacleCandidate[obsIndex].counter++;
