...
//set state of led, first 5 bits for 5 leds
...
//read switches, first 4 bits for each switch
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //call uartprint  and setled every 100 call for better performance
	if ((CpuTimer2.InterruptCount % 100) == 0) {
		UARTPrint = 1;
		SetLEDsOnOff(numTimer2calls);
		numTimer2calls++;
	    //if button 2 and 3 are pressed, stop counting
	    if ((read_button & 0x6) == 0x6){
	        numTimer2calls--;
	    }
	}
