...
uint16_t updownMotor = 0;
float EX2motorEffort = 0;
...
//    init_serialSCID(&SerialD,115200);
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
			...
			// UART_printfLine(1,"L:%.2f R:%.2f W:%.2f",left_encoder_reading, right_encoder_reading, wheel_reading);
			// UART_printfLine(2,"DL:%.2f DR:%.2f", left_dis, right_dis);
			UART_printfLine(1,"vL:%.2f vR:%.2f",v1 ,v2);
			UART_printfLine(2,"W:%.2f",wheel_reading);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer1_isr(void)
{
    ...
    if (v1 > 0.0) {
        uleft += 2.161 * v1 + 1.871;
    }
    else {
        uleft += 2.192 * v1 - 1.756;
    }

    if (v2 > 0.0) {
        uright += 2.161 * v2 + 1.871;
    }
    else {
        uright += 2.192 * v2 - 1.756;
    }
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    if (updown == 0) {
        ...
        if(EPwm12Regs.CMPA.bit.CMPA == 2500){
            updown=1;
    ...
    //ZW exercise 2 motor ramping up and down
    /*
    if (updownMotor == 0) {
        EX2motorEffort+=0.005;
        if(EX2motorEffort > 10){
            updownMotor=1;

        }
    }
    else if (updownMotor == 1) {
        EX2motorEffort-=0.005;;
        if(EX2motorEffort< -10){
            updownMotor=0;
        }
    }
    setEPWM1A(EX2motorEffort);
    setEPWM2A(EX2motorEffort);
    */
